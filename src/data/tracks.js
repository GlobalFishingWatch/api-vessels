...
const buildAdditionalFeature = options => ({
  ...
  concatCoordinateProperties: options.concatCoordinateProperties || (() => identity),
  extractAdditionalGeoJSONFeatures: options.extractAdditionalGeoJSONFeatures || (() => []),
});
...
const additionalFeaturesByName = {
  fishing: buildAdditionalFeature({
    ...
    extractAdditionalGeoJSONFeatures: (segments) => {
      const timedCoordinates = flatMap(segment => flow(
        zipAll,
        filter(([score]) => score > 0.5),
        map(([, time, coordinate]) => [time, coordinate]),
      )([segment.scores, segment.times, segment.geojson.coordinates]))(segments);
...
const segmentToGeoJSONTrackFeature = additionalFeatures => (segment) => {
  ...
  const coordinateProperties = reduce(
    (result, feature) => feature.concatCoordinateProperties(segment)(result),
...
const queryResultsToGeoJSON = (segments, additionalFeatures) => {
  ...
  const features = concat(
    ...
    flatMap(feature => feature.extractAdditionalGeoJSONFeatures(segments))(additionalFeatures),
  );
...
module.exports = dataset => ({
  forVessel(vesselId, additionalFeatureNames = []) {
    log.debug(`Looking up track for vessel ${vesselId} including features ${additionalFeatureNames}`);
    const additionalFeatures = map(name => additionalFeaturesByName[name])(additionalFeatureNames);
